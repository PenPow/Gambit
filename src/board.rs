...
impl Board {
	...
	pub fn make_move(&mut self, m: Move) -> bool {
		...
		if let Some(square) = self.state.en_passant_square {
			...
			self.state.zobrist_key ^= ZOBRIST_EN_PASSANT[ZOBRIST_EN_PASSANT.len() - 1]
		}
		...
		if piece != Pieces::PAWN {
			...
			if double_step {
				self.state.zobrist_key ^= ZOBRIST_EN_PASSANT[self.state.en_passant_square.unwrap_or(ZOBRIST_EN_PASSANT.len() - 1)];
	...
	fn display(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		...
		writeln!(f, "Side to Move: {}", Sides::as_str(self.state.side_to_move))?;
		writeln!(f, "En Passant Square: {}", if let Some(square) = self.state.en_passant_square { Squares::as_str(square) } else { String::from("-") })?;
		writeln!(f, "Castling Availability: {:b} (qkQK)", self.state.castling_availability)?;
		writeln!(f, "Half Move Clock: {}", self.state.half_move_clock)?;
		writeln!(f, "Full Move Timer: {}", self.state.full_move_number)?;		

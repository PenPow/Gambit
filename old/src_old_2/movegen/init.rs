use crate::{board::{bitboard::Bitboard, location::{Direction, Directions, Files, KnightJumps, Ranks, Square, Squares, FILE_BITBOARDS, RANK_BITBOARDS, SQUARE_BITBOARDS}, piece::{Piece, Pieces, Side, Sides}}, movegen::{piece_move::Move, NUMBER_OF_ROOK_MOVES}};
...
impl MoveGenerator {
	...
	pub(in crate::movegen) const fn init_pawn_captures() -> [MoveLookupTable; Sides::COUNT] {
		let mut moves: [MoveLookupTable; Sides::COUNT] = [[Bitboard::EMPTY; Squares::COUNT]; Sides::COUNT];

		let mut side: Side = 0;
		while side < Sides::COUNT { // i can't wait for the day I can finally use a for loop in a const fn
			let mut square: Square = 0;
			while square < Squares::COUNT {
				let (rank, file) = Squares::get_coordinates(square);
	
				let mut directions_index = 0;
				while directions_index < Directions::PAWN_CAPTURE_DIRECTIONS[side].len() {
					let direction = Directions::PAWN_CAPTURE_DIRECTIONS[side][directions_index];
					directions_index += 1;
					
					// TODO: Add comments
					if (rank == Ranks::R1)
						|| (rank == Ranks::R8)
						|| (file == Files::A && side == Sides::WHITE && (direction == Directions::NORTH_WEST))
						|| (file == Files::A && side == Sides::BLACK && (direction == Directions::SOUTH_WEST))
						|| (file == Files::H && side == Sides::WHITE && (direction == Directions::NORTH_EAST))
						|| (file == Files::H && side == Sides::BLACK && (direction == Directions::SOUTH_EAST))
					{
						continue;
					}
					...
					// More of a sanity check than anything else, its ran at compile time so the extra checks don't hurt
					if Squares::distance(square, new_square) == 1 {
						moves[side][square].0 |= SQUARE_BITBOARDS[new_square].0 // Working with the u64 directly since I can't mark my trait impls as const fns
					}
				}
